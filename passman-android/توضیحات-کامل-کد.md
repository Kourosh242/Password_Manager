...
> نسخهٔ فعلی: **1.0.2** (versionCode 3) — Android 10 تا 16 — همهٔ پردازندهها — کاملاً آفلاین
...
├── keystore/
│   ├── opm-release.keystore        ← کلید امضای release
│   └── keystore-info.txt           ← alias و رمزها
├── app/
...
android {
    ...
    signingConfigs {
        release {
            storeFile file('../keystore/opm-release.keystore')
            storePassword 'OpmSecure2026'
            keyAlias 'opm'
            keyPassword 'OpmSecure2026'
        }
    }
    buildTypes {
        release {
...
| `signingConfigs.release` | امضای خودکار بیلد release با کلید موجود در `keystore/` |
...
```
- امضا: خودکار با `keystore/opm-release.keystore` (alias `opm`) + zipalign + v1/v2/v3.
- **برای آپدیت روی نصب فعلی، حتماً با همین keystore امضا کنید** — اثر انگشت: `0cfbf68b6cd08f469fd471198456b23b87e2ad6673c6fc1d6444635a6a27868b`.
